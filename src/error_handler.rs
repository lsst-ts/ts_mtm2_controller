...
impl ErrorHandler {
    ...
    pub fn check_condition_power_system(
        ...
        self.check_digital_input_output(digital_input, digital_output);
    ...
    fn check_digital_input_output(&mut self, digital_input: u32, digital_output: u8) {
        // Active high inputs
        ...
        if (!(is_dc_ok_1 && is_dc_ok_2))
            || (self.config_power.is_boost_current_fault_enabled
                && (is_boost_current_on_1 || is_boost_current_on_2))
        {
...
mod tests {
    ...
    #[test]
    fn test_check_digital_input_output() {
        let mut error_handler = create_error_handler();

        // No fault
        error_handler.check_digital_input_output(
        ...
        assert!(error_handler.has_error(ErrorCode::FaultPowerSupplyLoadShare));
        assert!(error_handler.has_error(ErrorCode::FaultPowerHealth));
